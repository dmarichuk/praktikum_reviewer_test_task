...
class Record:
    def __init__(self, amount, comment, date=''): 
        ...
        # Дело в том, что стандартный интерпретатор питона - CPython - имеет встроенные объекты, которые создаются при запуске интерпритатора.
...
class Calculator:
    ...
    def get_week_stats(self):
        ...
        # А почему бы нам не использовать здесь list comprehension? Они же прекрасны! И в них можно проверять условия!
        for record in self.records:
            if (
...
class CashCalculator(Calculator):
    ...
    def get_today_cash_remained(self, currency,
        ...
        if cash_remained > 0:
            ...
            # 2 - До этого в коде для форматирования float встроенная функция round(), а здесь :.2f. Нужно выбрать что то одно 
